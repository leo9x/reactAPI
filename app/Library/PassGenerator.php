...
class PassGenerator
{
    ...
    public function addAssetViaUrl($type, $url)
    {
...
//        }
    ...
    private function createJsonManifest()
    {
        ...
        foreach ($this->assets as $filename => $path) {
            ...
            $hashes[$filename] = sha1($file_string);
            $this->assets[$filename] = $file_string;
        }
...
//         }
    ...
    private function zipItAll()
    {
        ...
        // Add all the assets
        foreach ($this->assets as $name => $file_string) {
            $zip->addFromString($name, $file_string);
        }
        $zip->close();
    }
    /*
